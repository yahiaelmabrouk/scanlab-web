    ...
    <v-expansion-panels v-model="trafficLightsPreviewPanel" accordion>
      <v-expansion-panel style="background-color: #fffef9">
        ...
        <v-expansion-panel-header>
          ...
          {{ isAdmin && scores.combinedScore >= 0 && trafficLightsPreviewPanel === 0 ? `(${overallScore}%)` : '' }}
        </v-expansion-panel-header>
        <v-expansion-panel-content class="admin-only-score-preview-content" v-if="scores">
          <b-tabs v-model="tabIndex" content-class="mt-3" nav-wrapper-class="sticky-top bg-white">
            ...
            <div v-if="isAdmin && scores.combinedScore >= 0 && trafficLightsPreviewPanel === 0">
              ...
              <tbody>
                <tr v-for="analysis in previewScoreAnalysisComputed" :key="analysis.keyName">
                  <td v-if="!isAdmin || !analysis.isBad" class="traffic-light-icon-holder">
                    <span v-if="analysis.color === 'green'" style="color: #006f00" class="traffic-light-icon">
                      <CheckboxMarkedCircleIcon />
                    </span>
                    <span v-else-if="analysis.color === 'yellow'" style="color: #b4b400" class="traffic-light-icon">
                      <AlertCircleIcon />
                    </span>
                    <span v-else-if="analysis.color === 'grey'" style="color: #808080" class="traffic-light-icon">
                      <AlertCircleIcon />
                    </span>
                  ...
                  <td v-else class="traffic-light-icon-holder">
                    ...
                    <span
                      ...
                      v-else
                    >
                      <span v-if="analysis.value === 'incomplete'" v-html="'&nbsp;---&nbsp;'"></span>
                      <span v-else>-{{ Math.round(analysis.scoreLoss) }}%</span></span
                    >
                  </td>
...
export default {
  ...
  computed: {
    ...
    previewScoreAnalysisComputed() {
      let output = []
      if (this.activePreviewGrades.groupScoreVariables) {
        output = this.activePreviewGrades.groupScoreVariables[this.previewScoreGroupIndex].analysis
      }
      return output
    },
    scores() {
      return this.previewScore?.sliceQuantScore || 0
    },
    ...
    activePreviewGrades() {
      ...
      switch (this.tabIndex) {
        case 0:
          output = this.scores.slicePrescriptionScore
          break
        case 1:
          output = this.scores.parameterScore
          break
        case 2:
          output = this.scores.imageResultScore
          break
        default:
