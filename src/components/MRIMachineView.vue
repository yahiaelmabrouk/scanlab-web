<template>
  <div class="mri-machine-container" :style="{ 'z-index': zIndex || 1, minHeight: '360px', maxHeight: maxHeight }">
    ...
    <template v-if="isShowControlSelectPatientPosition">
      <div class="top-right-action-control-board">
        <div class="checkbox-head-holder-container" v-if="isShowOptionHeadHolder">
          <v-checkbox v-model="isShowHeadHolder" :label="$t(getHeadholderTranslateKeyText())"></v-checkbox>
        </div>
    ...
    <!-- <template v-if="canShowHint && isShowHint && !isPreview">
      <div class="top-right-action-control-board">
        <div class="group-control">
          <button
            :class="{ active: hintDirection == hintDirectionTexts.AP }"
            @click="hintDirection = hintDirectionTexts.AP"
          >
            {{ $t('global.ap') }}
          </button>
          <button
            :class="{ active: hintDirection == hintDirectionTexts.LATERAL }"
            @click="hintDirection = hintDirectionTexts.LATERAL"
          >
            {{ $t('global.lateral') }}
          </button>
        </div>
    ...
    <template v-if="(canShowHint || canCheckLazerPosition) && !isPreview">
      <div class="body-box-controls" style="width: 187px">
        <template>
          ...
          <!-- <div v-if="testRunState === 'POSITION'">
            <button class="btn-machine-view" @click="onConfirmPosition()" :disabled="isConfirmPosition">
              Confirm position
            </button>
          </div>
          <div class="mt-2" v-else-if="testRunState === 'LANDMARK'">
            <button class="btn-machine-view" @click="onConfirmLandmark()" :disabled="isConfirmLandmark">
              Confirm landmark
            </button>
          </div> -->
        </template>
      ...
      <div class="body-box-controls">
        ...
        <template v-if="!isLocalizerBoxMode">
          ...
          <div class="group-scan-dir">
            <!-- <v-radio-group v-model="bodyBoxDirection">
    ...
    >
      <button class="float-button" @click="toggleLazer">{{ $t('global.toggle_laser') }}</button>
    </div>

    <div v-if="isAdmin && testRunState == 'QUESTIONS' && !isPreview && test" class="bottom-center-container">
      <p class="text-start">
        <strong>{{ `Profile:` }}</strong>
        {{
          patientPhysioInfoProfileName +
          (patientPhysioInfoCardiacLevelName ? ` (${patientPhysioInfoCardiacLevelName})` : '')
        }}
      </p>
    </div>
    ...
    <template v-if="isShowControlUpdown">
      <div class="key-action-control-board">
        <div></div>
        <button @mousedown="onActiveKeyCode('ArrowUp')">{{ `Up` }}</button>
        <div></div>
        <button @mousedown="onActiveKeyCode('ArrowLeft')" class="btn-up-down">{{ `In` }}</button>
        <button @mousedown="onActiveKeyCode('ArrowDown')">{{ `Down` }}</button>
        <button @mousedown="onActiveKeyCode('ArrowRight')" class="btn-up-down">{{ `Out` }}</button>
      </div>
      ...
      <div class="bottom-right-action-control-board">
        <v-menu top left>
          <template #activator="{ on }">
            <v-btn v-on="on"> {{ $t('global.view') }} </v-btn>
          </template>
          <v-list>
      ...
      <div class="p-2">
        ...
        <b-card>
          <div class="text-start">
            ...
            <!-- <span class="subtitle">This is predefined positions for the selected body part:</span> -->
          </div>
          <v-row class="mt-2 mx-0 mb-0">
            ...
            >
              {{ item.value.join(', ') }}{{ item.isShowHeadHolder ? `, ${$t(getHeadholderTranslateKeyText())}` : `` }}
            </v-col>
          </v-row>
        </b-card>
      </div>
...
import { getHeadholderTranslateKeyText } from '../util/utils'
...
const MIN_Z_OF_BED = -10
const MAX_Z_OF_BED = 10

const MAX_Y_OF_BED = -7.6
const MIN_Y_OF_BED = -35.7

const MAX_Y_IN_OUT_OF_BED = 180
...
export default {
  ...
  data() {
    return {
      ...
      injectorMachineGroup: new Group(),
      gltfLoader: null,
      injectorFileUrl: '/models/injector/Contrast_Media_Injector_MRI_opt.glb',
      injectorMachine: null,
    }
  },
  computed: {
    ...
    ...mapGetters('questionService', [
      ...
      'stackQuestion',
      'patientPhysioInfoProfileName',
      'patientPhysioInfoCardiacLevelName',
    ]),
    ...
    ...mapState('testRunService', ['test']),
    ...mapState('user', ['isAdmin']),
    modelFileUrl() {
      return `/models/${this.modelFileName}`
    },
    isVisibleInjectorMachine() {
      return this.testRunState == 'LANDMARK' || this.testRunState == 'POSITION'
    },
    ...
    isConfigLocalizerBox() {
      if (this.dicomFileSet) {
        return _.get(this.dicomFileSet, ['localizerBoundingBoxes', 'length'], 0) > 0
      }
      return false
    },
  },
  watch: {
    ...
    isVisibleInjectorMachine() {
      if (this.injectorMachineGroup) {
        this.injectorMachineGroup.visible = this.isVisibleInjectorMachine
      }
    },
  },
  async mounted() {
    ...
    this.gltfLoader.setDRACOLoader(drcLoader)
    ...
    this.innitInjectorMachineGroup()
    ...
    EventBus.$on('onCheckNeedToMoveMriUpDownToLandmarkPoint', this.checkNeedToMoveMriUpDownToLandmarkPoint)
  ...
  beforeDestroy() {
    ...
    EventBus.$off('onCheckNeedToMoveMriUpDownToLandmarkPoint', this.checkNeedToMoveMriUpDownToLandmarkPoint)
  ...
  methods: {
    ...
    innitInjectorMachineGroup() {
      this.injectorMachineGroup.children = []
      this.injectorMachineGroup.visible = this.isVisibleInjectorMachine
      this.injectorMachineGroup.position.set(90, 0, -40)
      this.injectorMachineGroup.rotation.set(0, Math.PI, 0)
      this.injectorMachineGroup.scale.set(0.7, 0.7, 0.7)
      this.scene.add(this.injectorMachineGroup)
    },
    ...
    setMriUpdownAndInOutPosition(machinePosition) {
      if (this.MriUpDownObject) {
        this.MriUpDownObject.updateWorldMatrix(true, true)
        const mriUpDownPositionY = _.get(machinePosition, ['mriUpDownPositionY'], 0)
        ...
        this.MriUpDownObject.position.set(
          this.MriUpDownObject.position.x,
          this.MriUpDownObject.position.y,
          this.MriUpDownObject.position.z + distanceY
        )
      }
      if (this.MriBedInOut) {
        this.MriUpDownObject.updateWorldMatrix(true, true)
    ...
    onMoveMRIUpdownToLowestPosition() {
      if (this.MriUpDownObject) {
        ...
        this.currentUpDownAnimation = gsap.to(this.MriUpDownObject.position, {
          z: MAX_Z_OF_BED,
          // Duration in setInterval auto play slice in SliceView
          duration: Math.abs(Math.abs(this.MriUpDownObject.position.z - MIN_Z_OF_BED)) / 30,
          ease: 'none',
          onComplete: () => {
            this.setMriMachineCurrentPos()
          },
    ...
    loadModels() {
      this.isLoading = true
      Promise.all([this.loadMriMachine(), this.loadInjectorMachine()])
        .then(() => {
    ...
    async loadMriMachine() {
      return new Promise((resolve) => {
        try {
          const onLoadModel = (object) => {
            MODELS_CACHE[this.modelFileUrl] = object
          ...
          if (MODELS_CACHE[this.modelFileUrl]) {
            ...
            this.gltfLoader.load(this.modelFileUrl, onLoadModel)
          }
        } catch (err) {
          // No action
          console.log('err', err)
    ...
    async loadInjectorMachine() {
      return new Promise((resolve) => {
        try {
          const onLoadModel = (object) => {
            MODELS_CACHE[this.injectorFileUrl] = object
            if (object?.scene) {
              if (this.injectorMachine) {
                this.injectorMachineGroup.remove(this.injectorMachine)
              }

              this.injectorMachine = object?.scene
              this.injectorMachineGroup.add(this.injectorMachine)
            }
            resolve()
          }

          if (MODELS_CACHE[this.injectorFileUrl]) {
            const object = MODELS_CACHE[this.injectorFileUrl]
            onLoadModel(object)
          } else {
            this.gltfLoader.load(this.injectorFileUrl, onLoadModel, undefined, (err) => {
              console.error('Error loading injector model:', err)
              resolve()
            })
          }
        } catch (err) {
    ...
    async onConfirmLandmark() {
      ...
      if (info) {
        ...
        if (LAZER_POSITION.y >= minRangeOfToleranceVerticalBox && LAZER_POSITION.y <= maxRangeOfToleranceVerticalBox) {
          ...
          // 10/13/2025 - We will allow 2cm of deviation in the Y direction before starting to lose points.
    ...
    checkNeedToMoveMriUpDownToLandmarkPoint() {
    ...
    setBodyBox(boxInfo, isSetUpDownPosition = false) {
      ...
      if (isSetUpDownPosition && this.MriUpDownObject) {
        const mriUpDownPositionY = _.get(boxInfo, ['mriUpDownPositionY'], 0)
        const currentUpDownPositionY = this.MriUpDownObject.getWorldPosition(new Vector3()).y
        const distanceY = currentUpDownPositionY - mriUpDownPositionY
