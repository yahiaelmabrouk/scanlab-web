<template>
  <div class="spin-btn-grp">
    <div class="spin-btn">
      <v-btn :disabled="disabled" @click="valueChange(sum(value, step, 4))"><v-icon>mdi-menu-up</v-icon></v-btn>
      <v-btn :disabled="disabled" @click="valueChange(subtract(value, step, 4))"><v-icon>mdi-menu-down</v-icon></v-btn>
    </div>
    <v-text-field
      :key="componentKey"
      :size="size"
...
export default {
  ...
  data() {
    return { componentKey: 0, lastEmittedValue: null }
  },
  props: {
    size: {
      type: String,
    ...
    value: {
      type: Number,
      default: 0, // Ensures it does not receive NaN or undefined
      required: true,
    },
    min: {
      type: Number,
