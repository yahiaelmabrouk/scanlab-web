<template>
  <div
    :class="[
      localSoftwareVersionPreference === 'xa' ? 'mr-0 ml-0 box-holder xa-box' : 'mr-1 ml-1 box-holder',
      amFullscreen ? 'fullscreen' : '',
  ...
  >
    <v-select
      ...
      hide-details
      :class="{
        'rounded-0 stack-config': localSoftwareVersionPreference === 'scanlab',
        'rounded-0 stack-config': localSoftwareVersionPreference !== 'xa',
        'rounded-0 mr-0 ml-0 xa-class': localSoftwareVersionPreference === 'xa',
        'black-background': localSoftwareVersionPreference === 'b19',
        'b19-selected': localSoftwareVersionPreference === 'b19',
        'r57-selected': localSoftwareVersionPreference === 'r57',
      }"
    ...
    <v-row cols="12" align="center" justify="space-between" class="m-1 d-flex" v-if="isFullscreen">
      ...
      <v-col v-if="allowSelection && showPointSelect && isCTLab">
        <div
          :class="['d-flex', 'align-items-center', { 'dot-size-slider-glow': !hasDotSizeSliderBeenClicked }]"
      ...
      <v-col col="4" class="d-flex align-center" v-else>
    ...
    <div class="box-container">
      ...
      <div class="inner-box-container" :style="computedStyle">
        <div
          ref="box"
          class="box"
          ...
          :class="{ 'selected-box': isSelectedSliceView }"
        ></div>
        <SliceViewContextMenu ref="contextMenu" @reset-segment="resetCameraPan" />
        <v-btn
          icon
          large
          color="buttonBlue"
        ...
        <p
          v-if="!isCTLab"
          class="orientation label-bottom-upper in-box-control clickable"
          @click="cycleThicknessMethod"
        >
          {{ thicknessMethodLabel }}
        </p>
        ...
        <brightness-auto class="bottom-left auto-windowing clickable in-box-control" @click="resetWindowing(true)" />
        <ContentCopyIcon
...
import SliceViewContextMenu from '@/components/SliceViewContextMenu.vue'
import {
  clamp,
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('selectionConfig', [
      ...
      'isHotkeysEnabledCT',
      'isParameterDisabled',
      'otherSelectionConfigInGroups',
      'orderedSliceViewComponentConfigs',
      'isSingleSliceMode',
    ]),
    ...
    ...mapGetters('timingDecisionService', ['isSelectedBolusTrackingTimingDecision']),
    ...mapState('interactableService', ['selectedInteractableIdent']),
    ...mapState('injectorService', ['injectorWaitingTime']),
    ...
    isSelectedSliceView() {
      return this.selectedSliceviewIds.includes(this.sliceId)
    },
    ...
    stackIdent() {
      // For multi-group merged scans, use the per-slice volume
      if (this.activeStackIdent) {
        return this.activeStackIdent
      }
    ...
    selectedInteractableInstance() {
      let ident = this.selectedInteractableIdent
      return _.find(this.interactableInstances, { ident })
    },
    disabledTools() {
      if (this.isCTLab) {
    ...
    currentSlabThicknessMethod() {
      // Get the thickness method for this specific slice view
      return this.slabThicknessMethod[this.sliceViewIndex] || 0
    },
    thicknessMethodLabel() {
      const methods = ['MIP', '2D', 'MinIP']
      return methods[this.currentSlabThicknessMethod] || 'MIP'
    },
    currentScanThickness() {
      // Get thickness from currently selected stackConfig's answerData
      return _.get(this.stackConfig, ['answerData', 'thickness']) || 0
    },
  },
  watch: {
    ...
    selectedStackQuestionIndex: 'onLoadInitialView',
    isSelectedBolusTrackingTimingDecision: function () {
      this.onSetSizeOfRenderer()
      this.resetCameraPan()
      this.geometryMustUpdate = true
      this.onLoadInitialView()
    },
  ...
  async mounted() {
    ...
    // This event make some event duplicated, so we disable it
    ...
    EventBus.$on(
      'cacheAllSliceViewConfigsBeforeEnteringFullscreen',
      this.onCacheAllSliceViewConfigsBeforeEnteringFullscreen
    )
  ...
  methods: {
    ...
    cycleThicknessMethod() {
      const currentMethod = this.currentSlabThicknessMethod
      const nextMethod = (currentMethod + 1) % 3
      // Update only this slice view's thickness method using Vue.set for proper reactivity
      this.$store.commit('selectionConfig/setSlabThicknessMethodForSlice', {
        sliceIndex: this.sliceViewIndex,
        method: nextMethod
      })
      // Directly update the stackHelper property
      this.stackHelper.slice.thicknessMethod = nextMethod
      // Flag material for update (required for shader uniform changes)
      this.stackHelper.slice.mesh.material.needsUpdate = true
      this.stackHelper.slice.mesh.material.uniformsNeedUpdate = true
      // Force immediate update of the slice rendering
      this.stackHelper.slice._update()
      this.render()
      // Trigger geometry update to maintain proper FOV and camera position
      this.geometryMustUpdate = true
    },
    ...mapMutations('pointSelectService', [
      ...
      'setUserPointSelectAnswerDotSize',
      'setHasDotSizeSliderBeenClicked',
    ]),
    ...
    ...mapActions('selectionConfig', [
      ...
      'updateBackgroundSlice',
      'clearGroupSelectionConfigs',
      'setFullScreenSourceId',
    ]),
    ...
    ...mapActions('questionService', ['captureDelayTimeFromTimingDecisionStart']),
    onLoadInitialView() {
      this.showStackByViewOrientation()
      this.onSetSelectionToSliceMidPointIfQuestionIsNew()
      this.updateShaderMaterialForCTMode()
    ...
    handleScanningChanged() {
      ...
      if (
        ...
        clearInterval(this.interval)
        this.interval = null
        if (this.timingDecisionStartTime && this.isAcquisitionQuestion) {
    ...
    getCenterPlaneIntersectionLine() {
      if (!this.stackHelper || !this.stackHelper.slice || !this.stackHelper.stack) {
        return null
      }
    ...
    onUpdateCenterPlaneIntersectionDir() {
      ...
      const handleEvent = () => {
        try {
          const dir = this.getCenterPlaneIntersectionLine()
          if (!this.amFullscreen && this.camera && dir) {
    ...
    resetWindowing(emitEvent = false) {
    ...
    refreshCameraFraming(setGeometryFlag = true) {
    ...
    onCacheAllSliceViewConfigsBeforeEnteringFullscreen() {
      if (!this.amFullscreen) {
        let componentConfig = {
          ...
          sourceId: this.id,
          componentSliceViewIndex: this.sliceViewIndex,
        }
        this.$store.dispatch('selectionConfig/cacheSliceViewConfigBeforeEnteringFullscreen', {
          componentConfig,
        })
      }
    },
    async toggleFullscreen() {
      ...
      if (!this.isFullscreen) {
        await this.$store.dispatch('selectionConfig/clearCachedSliceViewConfigsBeforeEnteringFullscreen')
        EventBus.$emit('cacheAllSliceViewConfigsBeforeEnteringFullscreen')
    ...
    onStackConfigChanged() {
      // Initialize to first slice's source volume, or null for single-group scans
      this.activeStackIdent = this.stackConfig?.slices?.[0]?.sourceStackIdent || null
      if (this.isCTLab) {
    ...
    onSetConfigFromComponentConfig(componentConfig) {
      this.windowWidth = _.get(componentConfig, 'windowWidth', this.windowWidth)
      this.windowCenter = _.get(componentConfig, 'windowCenter', this.windowCenter)
      this.geometryMustUpdate = true
      // if the stack was the same, still change the index
      this.setSliceIndex(componentConfig.indexSlice, true)

      this.zoom = componentConfig.zoom

      // Sync pan
      const currentContainerWidth = this.elBox.offsetWidth
      const currentContainerHeight = this.elBox.offsetHeight
      const oldContainerWidth = componentConfig.renderWidth
      const oldContainerHeight = componentConfig.renderHeight
      const oldCameraWidth = Math.abs(componentConfig.left - componentConfig.right)

      const oldRatio = oldContainerHeight / oldContainerWidth
      const currentRatio = currentContainerHeight / currentContainerWidth

      const newCameraWidth = (oldRatio * oldCameraWidth) / currentRatio

      const oldCameraToCenterX = (componentConfig.left + componentConfig.right) / 2
      const newCameraToCenterX = (currentRatio * oldCameraToCenterX) / oldRatio
      const isLeftLowerThanRight = this.camera.left < this.camera.right
      const newCameraLeft = newCameraToCenterX + ((isLeftLowerThanRight ? -1 : 1) * newCameraWidth) / 2
      const newCameraRight = newCameraToCenterX + ((isLeftLowerThanRight ? 1 : -1) * newCameraWidth) / 2
      const newCameraTop = componentConfig.top
      const newCameraBottom = componentConfig.bottom

      this.camera.left = newCameraLeft
      this.camera.right = newCameraRight
      this.camera.top = newCameraTop
      this.camera.bottom = newCameraBottom
    ...
    onFullscreenChange(event) {
      ...
      setTimeout(() => {
        try {
          ...
          if ((event.isFullscreen && this.amFullscreen) || event.targetId === this.id) {
            this.onSetConfigFromComponentConfig(event.componentConfig)
          ...
          log.error('Error onFullscreenChange', e)
        }
      }, 200)
    },
    onMouseWheel(event) {
      ...
      this.$emit('config-changed', {
        type: 'modify-slice-index',
        sliceId: this.sliceId,
        delta: event.deltaY > 0 ? 1 : -1,
      })
    },
    changeFrameByArrowIcon(offset) {
      if (this.orderedSliceViewComponentConfigs.length == 0) {
        ...
        const index = this.orderedSliceViewComponentConfigs.findIndex(
          (data) => data.stackConfig.name === this.stackConfig?.name
        )
        let componentConfig = this.orderedSliceViewComponentConfigs[index]
        if (index === 0 && offset === -1) {
          componentConfig = this.orderedSliceViewComponentConfigs[this.orderedSliceViewComponentConfigs.length - 1]
        } else if (index === this.orderedSliceViewComponentConfigs.length - 1 && offset === 1) {
          componentConfig = this.orderedSliceViewComponentConfigs[0]
        } else {
          componentConfig = this.orderedSliceViewComponentConfigs[index + offset]
        }

        this.stackConfig = componentConfig.stackConfig
        _.extend(this, componentConfig, {
          indexSliceOverwrite: componentConfig.indexSlice,
        })
        setTimeout(() => {
          this.onSetConfigFromComponentConfig(componentConfig)
        }, 50)
        this.setFullScreenSourceId(componentConfig.sourceId)
        this.geometryMustUpdate = true
      }
    },
    mouseDown(event) {
    ...
    mouseUp(event, shouldPreventDoubleClick) {
      ...
      if (event.button == 2 && this.sliceId != 'SliceView') {
        // Right click, show context menu
        const boudingRect = this.$refs.box.getBoundingClientRect()
        const x = event.clientX - boudingRect.left
        const y = event.clientY - boudingRect.top
        const distance = Math.sqrt(
          Math.pow(this.mouseDownPosition.x - event.clientX, 2) + Math.pow(this.mouseDownPosition.y - event.clientY, 2)
        )
        if (distance <= 1) {
          // If the mouse moved much, don't show context menu
          this.$refs.contextMenu.show(x, y, boudingRect)
        }
      }
      ...
      if (event.ctrlKey) {
        // Select this Slice View
        if (!_.some(this.stackVolumeSelections, 'isHoverDraggableDot')) {
          // Check if user not hover on any draggable dots of any StackVolumeSelection
          // We musst turn off group selection mode and select this SliceView
          if (this.groupSelectionConfigs.length > 0) {
            this.clearGroupSelectionConfigs()
            this.$emit('select-only', this.id)
          } else {
        ...
        if (distance < 5 && !_.some(this.stackVolumeSelections, 'isHoverDraggableDot')) {
          if (this.groupSelectionConfigs.length > 0) {
            this.clearGroupSelectionConfigs()
          }
          this.$emit('select-only', this.id)
    ...
    pointerMoveMain() {
      ...
      if (selectedObject && this.toolSelected === 'move' && this.mouseButtonsDown.left) {
        ...
        let planeClosest = this.getPlaneClosestOfSliceView()
        if (planeClosest.z && !selectedInteractableState) {
          const otherSliceViewsProjectionDirections = this.getOtherSliceViewsProjectionDirections()
          if (this.isSingleSliceMode && otherSliceViewsProjectionDirections.filter((el) => el && el.length() > 0)) {
            ...
            projectionMovements.forEach((el) => {
              ...
              if (!selectedInteractableState) {
                this.otherSelectionConfigInGroups.forEach((selection) => {
                  if (selection && selection.center3) {
                    selection.center3.add(el)
                  }
                })
              }
            })
            ...
            if (!selectedInteractableState) {
              this.otherSelectionConfigInGroups.forEach((selection) => {
                if (selection && selection.center3) {
                  selection.center3.add(movement3)
                }
              })
            }
          }
        ...
        const controlCenterPosition = selectedInteractableState
          ? this.selectedInteractableInstance.getControlCenterPosition()
          : this.selectedStackVolumeSelection.getControlCenterPosition()
        ...
        const isSingleDirectionExpansion = this.sliceExpansionBehavior == SLICE_EXPANSION_BEHAVIOR.SINGLE_DIRECTION
        if (isSingleDirectionExpansion) {
        ...
        if (
          ...
          // Always 2 side expansion in MR
          let alias = 1
          if (isSingleDirectionExpansion && !this.isCTLab) {
            alias = 2
          }
          this.mouseMovementResizeRemainder3.x += xDirMovement * alias * dimensionDiffMultiplier
          this.mouseMovementResizeRemainder3.y += yDirMovement * alias * dimensionDiffMultiplier
          let changed = {}
            ...
            if (isSingleDirectionExpansion && this.isCTLab) {
              haftOfMovement3.add(
                xDirMovement3
                  .clone()
                  .normalize()
                  .multiplyScalar(Math.abs(changedAmount) / 2)
              )
            }
          }
          //Y
        ...
        const updateConfigCenter3AfterModified = (move3) => {
          // Move box with haft movement
          if (selectionConfigCurrent && isSingleDirectionExpansion) {
            let { center3 } = selectionConfigCurrent
            center3.add(move3.clone()) // Maybe this should be a call to the selectionConfig store
            throttledDispatch(this.$store.dispatch, 'selectionConfig/afterSelectionUserModified', {})
          }
        }

        updateConfigCenter3AfterModified(haftOfMovement3)
        ...
        if (selectionConfigCurrent && zDirMovement !== 0) {
          ...
          const zMovementLength = zDirMovement3.length()
          const lastDimensionsZ = dimensions3.z
          ...
          this.$store.dispatch('selectionConfig/heightChanged', {
            ...
            onHeightChanged: () => {
              ...
              if (selectionConfigCurrent && isSingleDirectionExpansion) {
                const dimensionZDiff = Math.abs(dimensions3.z - lastDimensionsZ)
                const dimensionRate = dimensionZDiff / zMovementLength
                const adjustZMovement = zDirMovement3.clone().multiplyScalar(dimensionRate / 2)
                updateConfigCenter3AfterModified(adjustZMovement)
              }
            },
          })
        }
    ...
    setSliceIndex(index, updateGeometries = true) {
      ...
      // Multi-group volume swap: switch stackHelper when slice is from a different volume
      const sliceSourceIdent = this.stackConfigSlices[this.indexSlice]?.sourceStackIdent
      if (sliceSourceIdent && sliceSourceIdent !== this.activeStackIdent) {
        const previousHelper = this.stackHelper
        this.activeStackIdent = sliceSourceIdent
        if (previousHelper !== this.stackHelper) {
          this.stackHelperRemoveFromScene(previousHelper)
          this.stackHelperAddToScene()
          this.updateBackgroundSlice({
            sliceViewId: this.sliceViewId,
            backgroundSlice: this.stackHelper.slice,
          })
          _.each(this.stackVolumeSelections, (svs) => svs.setBackgroundSlice(this.stackHelper.slice))
          _.each(this.interactableInstances, (ii) => ii.setBackgroundSlice(this.stackHelper.slice))
          this.stackHelperPrevious = this.stackHelper
          // Invalidate limitedSquare cache — geometry was computed against a different volume
          if (this.stackConfig) {
    ...
    updateGeometries() {
      ...
      // For all non-CT scans, enable MPR Thick/MIP features
      if (!this.isCTLab && this.selectionConfigCurrent) {
        const thickness = this.currentScanThickness
        // Calculate spacing to ensure at least 2 MPR samples when thickness > 0
        // Formula: steps = floor(thickness/spacing + 0.5) >= 2 requires spacing <= thickness/1.5
        // Using thickness/2 guarantees steps >= 2, capped at 0.8 for performance
        const mprSpacing = thickness > 0 ? Math.min(0.8, thickness / 2) : 0.8

        if (this.stackHelper.slice.thickness !== thickness) {
          this.stackHelper.slice.thickness = thickness
        }
        if (this.stackHelper.slice.spacing !== mprSpacing) {
          this.stackHelper.slice.spacing = mprSpacing
        }
        if (this.stackHelper.slice.thicknessMethod !== this.currentSlabThicknessMethod) {
          this.stackHelper.slice.thicknessMethod = this.currentSlabThicknessMethod
        }
      } else {
        // For CT Lab: disable MPR by setting thickness to 0 (single 2D slice viewing)
        // Spacing must be non-zero to avoid division by zero in shader
        if (this.stackHelper.slice.thickness !== 0) {
          this.stackHelper.slice.thickness = 0
        }
        if (this.stackHelper.slice.spacing !== 1) {
          this.stackHelper.slice.spacing = 1
        }
      }
      ...
      if (dirAbs.x >= dirAbs.y && dirAbs.x >= dirAbs.z) {
        ...
        // Guard: dicomFileSet may be null/undefined during init
      ...
      if (this.isLimited && this.selectionCorners && this.stackVolumeSelections) {
        ...
        if (_.size(limitedSquare) > 2) {
          ...
          // MPR Blur Fix: Camera position must be updated when geometry changes
          // to prevent rendering artifacts (blur) in sagittal/coronal views.
          // Position camera at middle slice center (not current slice) for consistent MPR viewing.
          try {
            const centerSliceIndex = Math.floor(this.stackConfigSlicesAmount / 2) || 0
            // Only pre-compute center slice geometry if it uses the same volume as the current slice
            const centerSliceSourceIdent = this.stackConfigSlices[centerSliceIndex]?.sourceStackIdent
            if (!centerSliceSourceIdent || centerSliceSourceIdent === this.activeStackIdent) {
            ...
            if (limitedSquareOfMiddleSlice && _.size(limitedSquareOfMiddleSlice) > 2) {
              ...
              if (shapeGeoOfMiddleSlice) {
                if (!this.isMoveToCenterOfLimitSquare) {
                  // Initial setup: Use fitCameraToShape which calculates proper zoom
                  ...
                  // During interactions: Update camera position but preserve user's zoom
                  const middleMeshCenter = shapeGeoOfMiddleSlice.boundingBox.getCenter(new THREE.Vector3())

                  this.lastSetUpCameraStackConfigName = _.get(this.stackConfig, ['name'])
                  this.lastCameraOriginPosition.copy(middleMeshCenter)
                  this.camera.position.copy(middleMeshCenter)
                  ...
                  meshCenter = middleMeshCenter
                }
              }
            }
          } catch (err) {
            // Silently handle errors in camera positioning
          }

          // FOV LIMITING FIX: After createLimitedSquareForSliceIndex() calls _update() which corrupts
          // the current slice geometry, restore it. Only needed during interactions, not initial setup.
          if (this.isMoveToCenterOfLimitSquare) {
            const currentLimitedSquare = this.stackConfig.limitedSquare[this.indexSlice]
            if (_.size(currentLimitedSquare) > 2) {
              const restoredShapeGeo = this.createShapeGeometryFromLimitedSquare(currentLimitedSquare)
              this.stackHelper.slice.mesh.geometry.dispose()
              this.stackHelper.slice.mesh.geometry = restoredShapeGeo
              this.stackHelper.slice._geometry = restoredShapeGeo
              this.stackHelper.slice.geometry = restoredShapeGeo
            }
          }
        }
      }
    ...
    createShapeGeometryFromLimitedSquare(limitedSquare) {
      ...
      // MPR BLUR FIX: Add axis-aligned normals (from AMI.js geometries.slice.js)
      // This prevents blur artifacts in sagittal/coronal MPR views by ensuring
      // proper texture sampling aligned to the nearest axis.
      // Reference: GitHub issue FNNDSC/ami#392
      if (this.stackHelper?.slice?.planeDirection && limitedSquare.length >= 3) {
        const direction = this.stackHelper.slice.planeDirection
        ...
        // Create normal attribute using the snapped axis-aligned direction
        const normalArray = new Float32Array(limitedSquare.length * 3)
        for (let i = 0; i < limitedSquare.length; i++) {
          normalArray[i * 3] = snappedDir.x
          normalArray[i * 3 + 1] = snappedDir.y
          normalArray[i * 3 + 2] = snappedDir.z
        }

        newShapeGeo.setAttribute('normal', new THREE.Float32BufferAttribute(normalArray, 3))
      }
...
::v-deep .b19-selected .v-input--is-focused .v-select__selections,
::v-deep .b19-selected .v-select__selection {
  color: white !important;
}
::v-deep .xa-class .v-input--is-focused .v-select__selections,
::v-deep .xa-class .v-select__selection {
  color: rgb(0, 0, 0) !important;
}
::v-deep .r57-selected .v-input--is-focused .v-select__selections,
::v-deep .r57-selected .v-select__selection {
  color: rgb(0, 0, 0) !important;
}

::v-deep .b19-selected .theme--light.v-icon {
  color: white !important;
}
...
.non-scanlab-box ::v-deep .v-select.v-input--dense .v-select__selection--comma {
  color: white !important;
}

.non-scanlab-box ::v-deep .v-select__slot .v-input__append-inner .v-icon {
  color: white !important;
}
</style>
