<template>
  <v-card
    v-if="!isFreebie && (isManager || isAdmin || isSandbox)"
    ...
    <v-expansion-panels v-model="trafficLightsPreviewPanel" accordion>
      <v-expansion-panel style="background-color: #fffef9">
        ...
        <v-expansion-panel-content class="admin-only-score-preview-content">
          <div v-if="previewScore && previewScore.sliceQuantScore">
            <b-tabs
              content-class="mt-3"
              v-if="
                previewScore &&
                previewScore.sliceQuantScore &&
                previewScore.sliceQuantScore.slicePrescription &&
                previewScore.sliceQuantScore.slicePrescription.sliceGroups &&
                previewScore.sliceQuantScore.slicePrescription.sliceGroups.length > 1
              "
            >
              <b-tab
                v-for="groupVars in previewScore.sliceQuantScore.slicePrescription.sliceGroups"
                :key="groupVars.groupScoreVariables.userAnswerConfigIndex"
                :title="groupVars.groupScoreVariables.userAnswerConfigIndex + 1 + ''"
                :active="groupVars.groupScoreVariables.userAnswerConfigIndex === previewScoreGroupIndex"
                :class="'bold'"
                @click="previewScoreGroupIndex = groupVars.groupScoreVariables.userAnswerConfigIndex"
              ></b-tab>
            </b-tabs>
                ...
                <tr v-for="analysis in previewScoreAnalysisComputed" :key="analysis.keyName">
                  <template>
                    ...
                    <td v-if="!isAdmin || !analysis.isBad" class="traffic-light-icon-holder">
                      <span v-if="analysis.color === 'red'" style="color: #b60000" class="traffic-light-icon">
                        <CloseCircleIcon />
                      </span>
                    ...
                    <td v-else class="traffic-light-icon-holder">
                      <span
                        ...
                        v-if="isOnlyShowRightOrWrong(analysis.factorName)"
                      >
                        <CloseCircleIcon />
                      </span>
                      <span
  ...
  computed: {
    ...
    previewScoreAnalysisComputed() {
      ...
      if (
        s &&
        s.slicePrescription &&
        s.slicePrescription.sliceGroups &&
        s.slicePrescription.sliceGroups.length > this.previewScoreGroupIndex
      ) {
        return s.slicePrescription.sliceGroups[this.previewScoreGroupIndex].groupScoreVariables.analysis || []
      }
      return []
    },
    scores() {
      return this.previewScore && this.previewScore.sliceQuantScore ? this.previewScore.sliceQuantScore : 0
    },
    overallScore() {
      return _.round(this.scores.combinedScore, 2)
    },
  },
  methods: {
